 ...
 * @author Sean Owen
 */
public final class DataMatrixMaker {
	...
	private DataMatrixMaker() {
	}

	// added in for time testing
	public static void main(String[] args) throws Exception {
		...
		BitMatrix matrix = new MultiFormatWriter().encode(args[0],
				barcodeFormat, width, height);
		MatrixToImageWriter.writeToPath(matrix, imageFormat,
				Paths.get(outFileString));
		
		long delta = System.currentTimeMillis() - startTime;
		System.out.println("Time is " + delta + " ms");
	...
	private static void printUsage() {
		...
		System.err
				.println("usage: CommandLineEncoder [ options ] content_to_encode");
		System.err
				.println("  --barcode_format=format: Format to encode, from BarcodeFormat class. "
						+ "Not all formats are supported. Defaults to QR_CODE.");
		System.err
				.println("  --image_format=format: image output format, such as PNG, JPG, GIF. Defaults to PNG");
		System.err
				.println("  --output=filename: File to write to. Defaults to out.png");
